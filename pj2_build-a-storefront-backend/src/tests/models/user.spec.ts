...
describe('User Model', () => {
  ...
  it('should return a list of users', async () => {
    ...
    expect(result[0].username).toEqual('NhatMQ');
    expect(result[0].id).toEqual(1);
    expect(result[0].firstname).toEqual('Nhat');
    expect(result[0].lastname).toEqual('Mai');
  });
  ...
  it('should remove the user', async () => {
    const createdUser: User = await createUser(user);
    await deleteUser(createdUser.id);
    expect(createdUser.firstname).toEqual('Nhat');
    expect(createdUser.lastname).toEqual('Mai');
  });
